...
- 关闭页面或网络断开时，回答仍在服务端继续生成，页面恢复后自动重连。其他打开的窗口会在几秒内看到新问题、分支、设置和正在生成的回答。
- 通过私有的 Tailscale 网络在手机上打开同一个学习空间：「设置 → 手机访问」给出一次性设置命令和二维码。触屏上选中文字后，操作按钮停靠在屏幕底部；输入框字号避免 iOS 聚焦时放大页面。详见[手机访问](docs/operations.md#phone-access)。
...
- 支持 OpenAI 兼容 Chat Completions 和 Anthropic 原生 Messages；可选 MCP 提供联网搜索（含网页正文阅读）、学习资料、知识记忆、浏览器等工具。
...
| 编辑模型密钥 | 留空保留原密钥。 |
| 上下文预算 | 在模型「高级设置」填写实际窗口和回答预留；长对话按需使用当前模型生成摘要，每次提问最多新增一次摘要调用，缓存复用，失败回退本地摘录，无需下载模型。 |
...
在「设置 → 记忆」里，已有偏好会先合并显示，打开设置不会修改原记录。保存后，大框由你管理，AI 不会改写；下方默认折叠的「AI 补充」仍可学习明确表达的新偏好，支持查看来源、全局/话题范围、编辑和删除。与你手写内容冲突的建议不会自动生效，开关可控制今后的自动记忆；删除后不会从同一个问题重新提取。所有设置页共用固定外框，长内容在框内滚动。修改话题事实会保留来源并更新检索缓存；管理界面的跨主题搜索不会扩大聊天可引用的学习路径。详见[记忆管理](docs/memory-management.md)。
...
长对话另有逐请求预算检查，工具返回也计入估算。较早对话使用带来源的模型摘要与原句摘录，当前问题、引用、笔记和能容纳的近期完整问答保留原文。摘要会增加少量模型调用费用及等待时间，成功后缓存复用；原始聊天保持不变，删除来源时同步删除相关摘要。估算不是服务商的精确 token 计数，压缩也可能遗漏相关内容。详见[上下文预算、原文回读与限制](docs/context-budget.md)。
...
当前没有账户鉴权，MCP 能运行本机程序。启动命令只监听 `127.0.0.1`，适用于个人本机使用；公网、多用户部署需要另行增加访问控制，不属于本版支持范围；手机访问请只通过 [Tailscale Serve](docs/operations.md#phone-access) 这类私有网络。参见 [安全说明](SECURITY.md)。
